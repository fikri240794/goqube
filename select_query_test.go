...
// TestSelectQuery_BuildSelectQuery tests the BuildSelectQuery method for different SQL dialects and scenarios.
// It verifies correct SQL generation, argument handling, and error responses for supported and unsupported dialects.
func TestSelectQuery_BuildSelectQuery(t *testing.T) {
	tests := []struct {
		name       string
		dialect    Dialect
		expectErr  error
	...
	// Create a simple select query for testing.
	q := &SelectQuery{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Build the select query for the given dialect.
			sql, args, err := q.BuildSelectQuery(tt.dialect)
			if tt.expectErr != nil {
