...
func Test_sqlServerBuilder_BuildBulkUpdateQuery(t *testing.T) {
	b := newSQLServerBuilder()
	tests := []struct {
		name     string
		q        *BulkUpdateQuery
		...
		},
		{
			...
			wantSQL:  "UPDATE t SET age = c.age, name = c.name FROM users AS t INNER JOIN (VALUES (CONVERT(int, @p0), CONVERT(int, @p1), CONVERT(nvarchar(max), @p2)), (CONVERT(int, @p3), CONVERT(int, @p4), CONVERT(nvarchar(max), @p5))) AS c(id, age, name) ON t.id = CONVERT(int, c.id)",
		...
		},
		{
			...
			wantSQL:  "UPDATE t SET age = c.age, name = c.name FROM users AS t INNER JOIN (VALUES (CONVERT(bigint, @p0), CONVERT(tinyint, @p1), CONVERT(varchar(100), @p2)), (CONVERT(bigint, @p3), CONVERT(tinyint, @p4), CONVERT(varchar(100), @p5))) AS c(id, age, name) ON t.id = CONVERT(bigint, c.id)",
			wantArgs: []interface{}{1, 30, "foo", 2, 40, "bar"},
			wantErr:  false,
		},
		{
