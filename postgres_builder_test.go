...
func TestPostgresBuilder_BuildBulkUpdateQuery(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantSQL:  "UPDATE users AS t SET age = c.age, name = c.name FROM (VALUES ($1::integer, $2::integer, $3::text), ($4::integer, $5::integer, $6::text)) AS c(id, age, name) WHERE t.id = c.id::integer",
		...
		},
		{
			...
			wantSQL:  "UPDATE products AS t SET active = c.active, name = c.name, price = c.price FROM (VALUES ($1::integer, $2::boolean, $3::text, $4::double precision), ($5::integer, $6::boolean, $7::text, $8::double precision)) AS c(id, active, name, price) WHERE t.id = c.id::integer",
