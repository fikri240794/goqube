...
// TestDeleteQuery_BuildDeleteQuery tests the BuildDeleteQuery method for different SQL dialects and scenarios.
// It checks for correct SQL generation, argument handling, and error responses for supported and unsupported dialects.
func TestDeleteQuery_BuildDeleteQuery(t *testing.T) {
	tests := []struct {
		name       string
		dialect    Dialect
		expectErr  error
	...
	// Create a simple delete query for testing.
	q := &DeleteQuery{Table: "users"}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Build the delete query for the given dialect.
			sql, args, err := q.BuildDeleteQuery(tt.dialect)
			if tt.expectErr != nil {
